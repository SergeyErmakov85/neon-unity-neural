...
n_states = env.observation_space.n  # Количество состояний (500)`}</CodeBlock>

          <h3 className="text-xl font-semibold text-foreground mt-8 mb-3">2. Определение нейронной сети</h3>
          <CodeBlock>{`class DQN(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.out(x)       # Q-значения для каждого действия`}</CodeBlock>
          ...
          <CodeBlock>{`# Инициализация сети, оптимизатора и параметров
...
def select_action(state):
    if np.random.rand() < epsilon:
        ...
        with torch.no_grad():
            ...
            return torch.argmax(policy_net(state_tensor)).item()`}</CodeBlock>
